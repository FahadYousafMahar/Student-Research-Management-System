          ...
          <div class="tabbable paper-shadow relative" data-z="0.5">
            ...
            <div class="tab-content">
              ...
              <div id="course" class="tab-pane active">
                <form action="/editapaper/<?= $paper->id ?>" method="post" enctype="multipart/form-data" >
                  <div class="form-group form-control-material">
                    <input type="text" name="title" id="title" value="<?= $paper->title?>" placeholder="Title" class="form-control used"><span class="ma-form-highlight"></span><span class="ma-form-bar"></span>
                    <label for="title">Title of Research Paper</label>
                  </div>
                  <div class="form-group">
                    <label for="body">Summary</label>
                    <textarea id="summernote" name="body"><?= $paper->body?></textarea>
                  </div>
                  <div class="form-group">
                  <span>Select Supervisor</span>
                  <select id="selector" name="stdid">
                  <?php
                  if(count($students>0)){
                  foreach ($students as $i) {?>
                  <option value="<?= $i->id ?>"  <?= ($i->id== $paper->stdid)?'selected':''; ?>><?= ucwords($i->fname).' '.ucwords($i->lname)?></option>
                  <?php }} ?>
                  </select>
                  </div>
                  <div class="form-group">
